export const BIOGRAPHY_DATA = [
  {
    period: '1943 - 1956',
    title: 'Origines et Éveil',
    events: [
      "En 1943, je suis né à Edou, à quelques encablures de la localité d’Oyo, dans le département de la Cuvette, au nord du Congo.",
      "En 1956, après avoir fréquenté l’école primaire de mon village et celle de Fort-Rousset (Owando), je suis admis au concours d’entrée au collège normal Raymond Paillet de Dolisie. Très tôt, je m’intéresse à la vie politique et acquiers une conscience idéologique progressiste au sein de l’association scolaire du Congo (ASCO)."
    ],
    images: [
      "../../../../../../images/biography/origins/congo-map.jpg",
      "../../../../../../images/denis.webp",
      "../../../../../../images/jeunes-congolais.webp",
      "../../../../../../images/parti-congolais-du-travail.webp",
    ]
  },
  {
    period: '1960 - 1968',
    title: 'La Vocation des Armes',
    events: [
      "En 1960, sur les conseils de mon professeur Maurice Spindler, je me présente au concours d’entrée à l’école inter-armes de Cherchell en Algérie, d’où je sors major de promotion. Je fais partie des premiers officiers qui vont encadrer la nouvelle armée congolaise.",
      "En 1963, après une formation à Saint-Maixent en France, je deviens officier des troupes aéroportées et témoin privilégié de la révolution des 13, 14 et 15 août.",
      "En 1968, promu Capitaine, je participe au mouvement du 31 juillet avec Marien Ngouabi, aboutissant à la création du Parti Congolais du Travail (PCT) dont je suis membre fondateur."
    ],
    images: [
      "../../../../../../images/biography/arms/sassou.avif",
      "../../../../../../images/parti-congolais-du-travail.webp",
      "../../../../../../images/biography/arms/sassou-pct.jpg",
      "../../../../../../images/biography/arms/pct.jpg",
      "../../../../../../images/biography/arms/pct-sassou.png",
    ]
  },
  {
    period: '1975 - 1979',
    title: 'L’Appel des Responsabilités',
    events: [
      "En décembre 1975, je suis reconduit comme Secrétaire du Comité Central chargé de la commission permanente à l’armée lors d'un réaménagement du directoire politique.",
      "En 1977, après la disparition tragique du Président Marien Ngouabi, je deviens Vice-président du Comité Militaire du Parti (CMP), Ministre de la défense et de la sécurité.",
      "Le 5 février 1979, je suis élu Président du Comité Central du PCT. Le Congrès de mars 1979 confirme mon élection à la tête du Parti et de l’État."
    ],
    images: [
      "../../../../../../images/biography/responsabilities/young-sassou.jpeg",
      "../../../../../../images/biography/responsabilities/sassou-young.jpg",
      "../../../../../../images/biography/responsabilities/sassou.webp",
    ]
  },
  {
    period: '1987 - 1991',
    title: 'Le Rayonnement Africain',
    events: [
      "En 1987, élu Président de l’OUA, j’initie le Fonds Africa et le symposium des écrivains contre l’apartheid.",
      "Le 13 décembre 1988, la signature du protocole de Brazzaville consacre l’indépendance de la Namibie et la libération de Nelson Mandela, marquant l'histoire de la nouvelle Afrique du Sud.",
      "En 1990 et 1991, j'ouvre le pays au multipartisme et accepte la tenue de la conférence nationale souveraine, me retirant du pouvoir en 1992 pour favoriser l’alternance."
    ],
    images: [
      "../../../../../../images/biography/shine/oua.jpg",
      "../../../../../../images/biography/shine/sassou-mandela.jpg",
      "../../../../../../images/biography/shine/alternance.webp",
    ]
  },
  {
    period: '1997 - 2006',
    title: 'Réconciliation et Reconstruction',
    events: [
      "De 1997 à 2001, après une période de guerre civile, je reviens au pouvoir avec l'ambition de réconcilier le peuple via le Forum National et le Dialogue National Sans Exclusive.",
      "Le 20 janvier 2002, j’organise le référendum adoptant la nouvelle constitution, suivi de mon élection à la Présidence de la République le 10 mars.",
      "En 2003 et 2006, je porte le « Pacte de non-Agression » de l’Union Africaine et assume pour la deuxième fois la présidence en exercice de l’UA."
    ],
    images: [
      "../../../../../../images/biography/back/referendum.avif",
      "../../../../../../images/biography/back/dialog.jpg",
      "../../../../../../images/biography/back/sassou.webp",
    ]
  },
  {
    period: '2009 - 2013',
    title: 'Stabilité et Émergence',
    events: [
      "Le 12 juillet 2009, je suis réélu Président de la République pour un mandat de sept ans.",
      "De 2010 à 2013, j'assume la présidence de la CEMAC, je porte la voix de l'Afrique au Sommet de Rio+20, et je suis désigné Médiateur pour la crise en République Centrafricaine."
    ],
    images: [
      "../../../../../../images/biography/arise/rio.jpg",
      "../../../../../../images/biography/arise/rca-congo.jpg",
      "../../../../../../images/biography/arise/cemac.jpg",
    ]
  },
  {
    period: '2014 - 2021',
    title: 'La Marche vers la 6ème République',
    events: [
      "Entre 2014 et 2015, après le Dialogue de Sibiti, le peuple adopte par référendum une nouvelle Constitution le 25 octobre, consacrant l’entrée dans la 6ème République.",
      "En mars 2016, je suis élu Président de la Nouvelle République. En 2017, je reçois la Médaille Amílcar Cabral pour mon rôle de médiateur en Guinée Bissau.",
      "En mars 2021, je suis réélu pour un nouveau mandat de 5 ans avec pour missions clés la préservation du fleuve Congo et la redynamisation de l’économie post-pandémie."
    ],
    images: [
      "../../../../../../images/biography/sixth/corona.jpg",
      "../../../../../../images/biography/sixth/cabrale.jpg",
      "../../../../../../images/biography/sixth/new.jpeg",
    ]
  }
];